...
def build_system_prompt(
    ...
    return (
        ...
        "### DUBL SCORES — REFLECT ACTUAL CONVICTION\n"
        "Score each pick 1.0-5.0 using 0.1 precision (e.g. 2.3, 3.7, 4.1). "
        "The score reflects how strong the EVIDENCE you found is — not a forced distribution. "
        "If your search turned up strong ATS records, a clear injury edge, and a PPG mismatch, score it high. "
        "If you're picking a side but the data is mixed or thin, score it low. "
        "Some nights zero games deserve a 4.0+. Other nights three might. Let the evidence decide. "
        "Do NOT default to 4.0 or 3.5 as a safe middle — those scores should mean something specific.\n\n"
...
async def analyze_game(req: AnalyzeRequest):
    ...
    if is_live:
        prompt = (
            ...
            "DUBL_SCORE_BET: [float 1.0-5.0 in 0.1 increments. Score based on how clear the live edge is — "
            "obvious momentum/blowout = high, tight/uncertain = low. Do NOT default to 4.0.]\n"
            "DUBL_REASONING_BET: [1 sentence about the EXACT bet — reference the live score situation, momentum, or specific matchup factor.]\n"
            "DUBL_SCORE_OU: [float 1.0-5.0 in 0.1 increments. Score based on how clearly the current pace "
            "projects over/under the line. Big gap = high, close = low.]\n"
        ...
        prompt = (
            ...
            "DUBL_SCORE_BET: [float 1.0-5.0 in 0.1 increments. Score based purely on the strength of evidence "
            "you found for this pick. Strong ATS record + clear matchup edge + situational factor = high score. "
            "Mixed signals or thin data = low score. Do NOT default to 4.0 as a safe middle.]\n"
            ...
            "DUBL_SCORE_OU: [float 1.0-5.0 in 0.1 increments. Score based on how far the PPG projection is from the line "
            "and how strong the supporting evidence is. Big mismatch with supporting factors = high. Close call = low.]\n"
