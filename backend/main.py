...
async def analyze_game(req: AnalyzeRequest):
    ...
    if is_live:
        prompt = (
            ...
            "RULE: If the spread is 3 points or fewer, ALWAYS pick the underdog ML — a tiny spread gives almost no protection so the ML at a better price is strictly superior to paying -110 juice. "
            "Only pick spread if: spread is 4+ points AND the favorite is -200 or worse. "
        ...
        prompt = (
            ...
            "OU_LINE: [current O/U total from your search, e.g. 228.5]\n"
            "BEST_BET: [Pick the AWAY_ML, HOME_ML, or SPREAD_LINE you wrote above — NEVER a player prop. "
            "RULE: If the spread is 3 points or fewer, ALWAYS pick the underdog ML — a 1.5 or 2.5 point spread gives almost zero protection so the ML at +100 or better is strictly superior to paying -110 juice for the spread. "
            "Only pick spread if: (1) the spread is 4+ points AND the favorite is -200 or worse, making -110 juice much better value. "
