...
// Report game result manually (for BGA or when auto-poll fails)
// One player reports, the other confirms. If both agree, wager is settled.
router.post('/:id/report-result', async (req: Request, res: Response) => {
  const userId = req.user!.userId;
  const { winnerId } = req.body; // winnerId or 'draw'
  const ref = db.collection('dublplay_wagers').doc(String(req.params.id));
  const doc = await ref.get();
  ...
  // Validate winnerId
  const validWinners = [w.challengerId, w.opponentId, 'draw'];
  if (!validWinners.includes(winnerId)) {
    throw new AppError(400, 'Invalid winnerId');
  }

  const now = new Date().toISOString();

  // Check if the OTHER player already reported
  if (w.reportedResult && w.reportedBy !== userId) {
    // Other player already reported — check if they agree
    if (w.reportedResult === winnerId) {
      // Both agree! Settle the wager
      const result = winnerId === 'draw' ? 'draw'
        : winnerId === w.challengerId ? 'challenger_win' : 'opponent_win';
      const actualWinnerId = winnerId === 'draw' ? null : winnerId;
      ...
      await ref.update({
        ...
        settledAt: now,
        updatedAt: now,
        settledBy: 'manual_report',
      });

      // Process payout/refund
      if (actualWinnerId) {
        // Pay winner
        const winnerRef = db.collection('dublplay_users').doc(actualWinnerId);
        ...
        await winnerRef.update({ walletBalanceCents: currentBalance + totalPot, updatedAt: now });
        await db.collection('dublplay_transactions').doc().set({
          wagerId: doc.id, userId: actualWinnerId, type: 'payout', amountCents: totalPot, status: 'completed', createdAt: now,
        });
      } else {
        // Draw — refund both
        for (const uid of [w.challengerId, w.opponentId]) {
          const userRef = db.collection('dublplay_users').doc(uid);
          const userDoc = await userRef.get();
          const currentBalance = userDoc.data()?.walletBalanceCents || 0;
          await userRef.update({ walletBalanceCents: currentBalance + w.amountCents, updatedAt: now });
          await db.collection('dublplay_transactions').doc().set({
            wagerId: doc.id, userId: uid, type: 'draw_refund', amountCents: w.amountCents, status: 'completed', createdAt: now,
          });
        }
      }
      ...
      // Disagreement — clear previous report, let them try again
      await ref.update({
        reportedResult: winnerId,
        reportedBy: userId,
        reportDisagreement: true,
        updatedAt: now,
      });
      res.json({ reported: true, disagreement: true, message: 'You and your opponent reported different results. Your report has been updated. Ask your opponent to confirm.' });
      return;
    }
  }

  // First report — save it and wait for other player to confirm
  await ref.update({
    reportedResult: winnerId,
    reportedBy: userId,
    updatedAt: now,
  ...
  res.json({ reported: true, message: 'Result reported. Waiting for opponent to confirm.' });
});
