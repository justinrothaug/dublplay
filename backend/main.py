...
def build_system_prompt(
    ...
    return (
        ...
        "TOTALS / OVER-UNDER — ANALYZE THE ACTUAL GAME:\n"
        "The O/U pick must be based on analyzing THESE TWO TEAMS' matchup, not generic rules.\n"
        "• STEP 1: Look at each team's PPG and OPP PPG from the TEAM CONTEXT above. Add Team A's PPG + Team B's PPG "
        "to estimate raw combined scoring. Compare this to the posted O/U line.\n"
        "• STEP 2: Factor in pace — are both teams fast (top 10 pace)? Both slow? Mismatch? "
        "High-pace matchups (200+ combined possessions) push OVER; both slow = UNDER.\n"
        "• STEP 3: Check defensive ratings — if one team allows 115+ ppg, the other team's offense will feast → OVER. "
        "If both are top-10 defenses (<108 opp ppg), UNDER.\n"
        "• STEP 4: Apply situational adjustments ON TOP of the matchup analysis:\n"
        "  - B2B fatigue drags scoring down 3-5% → nudge toward UNDER\n"
        "  - Double-digit spreads hit the over 60.7% (blowouts produce garbage time points)\n"
        "  - Teams shooting 5%+ above season TS% will regress → fade hot shooting\n"
        "  - Key offensive player out → lower total; key defensive player out → higher total\n"
        "• The O/U reasoning MUST reference specific numbers (ppg, opp ppg, combined total vs the line). "
        "Do NOT just cite a generic rule like 'double-digit spreads hit overs'. Explain WHY this specific game goes over or under.\n\n"
...
async def analyze_game(req: AnalyzeRequest):
    ...
    if is_live:
        prompt = (
            ...
            "DUBL_SCORE_BET: [float 1.0-5.0 — use this rubric:\n"
            "  1.0-2.0 = coin flip, no clear edge, conflicting signals\n"
            "  2.5-3.0 = slight lean, one factor supports the pick\n"
            "  3.5 = solid edge, strong ATS record or one clear situational factor\n"
            "  4.0 = STRONG — one clear situational edge (B2B fade, bounce-back, letdown, key injury) backed by supporting data (ATS, form, matchup)\n"
            "  4.5 = VERY STRONG — a situational edge with multiple supporting factors, or two situational edges combined\n"
            "  5.0 = near-lock, dominant spot with overwhelming data support. Rare but don't be afraid to use it.\n"
            "  Expect 2-4 picks per night to reach 4.0+. These are the top picks users should prioritize.]\n"
            "DUBL_REASONING_BET: [1 sentence about the EXACT bet you chose in BEST_BET — if you picked the spread, explain the spread; if you picked the ML, explain the ML. Do NOT mention the other bet type. Name the situational edge if one applies.]\n"
            "DUBL_SCORE_OU: [float 1.0-5.0 — same rubric as above but for O/U. Score 4.0+ when the matchup data (PPG, pace, defensive ratings) clearly supports the lean AND a situational factor aligns.]\n"
            "DUBL_REASONING_OU: [1 sentence: reference SPECIFIC numbers — combined PPG vs the line, pace, or defensive ratings. Do NOT just name a generic rule.]"
        )
    else:
        prompt = (
            ...
            "Consider two separate questions: (1) Who wins? (2) Does the underdog COVER the spread? "
            "A team can lose the game but still cover a large spread (e.g. losing by 8 when getting +15.5 is a cover). "
            "For spreads of 10+ points, seriously consider the underdog side — big favorites often win but fail to cover. "
            "Check ATS (against-the-spread) records: a team that covers frequently is a better spread bet even if they lose outright. "
            "For ML: only pick ML when the price is reasonable (better than -200). Prefer SPREAD over expensive ML. "
            "Format: 'TEAM LINE — 2-sentence reason (who has the edge and why, then why this bet type).']\n"
            f"BET_TEAM: [{game['away']} or {game['home']} — abbreviation only]\n"
            "BET_TYPE: [SPREAD or ML — which did you recommend in BEST_BET?]\n"
            "OU_LEAN: [Use the OU_LINE you wrote above. ANALYZE THIS GAME: Look up each team's PPG and OPP PPG. "
            "Estimate the combined scoring based on Team A's offense vs Team B's defense and vice versa. Compare to the O/U line. "
            "Then layer on any situational edge (B2B, double-digit spread, key injury). "
            "Format: 'OVER/UNDER [that number] — 2-sentence reason: first explain the matchup-specific scoring projection with real numbers, then note any situational factor']\n"
            "PLAYER_PROP: [Player prop line from your search. Format: 'Player OVER/UNDER X.X Stat — 1 sentence reason']\n"
            "DUBL_SCORE_BET: [float 1.0-5.0 — use this rubric:\n"
            "  1.0-2.0 = coin flip, no clear edge, conflicting signals\n"
            "  2.5-3.0 = slight lean, one factor supports the pick\n"
