...
def build_system_prompt(
    ...
    return (
        ...
        "### DUBL SCORES — ADDITIVE RUBRIC (start at 1.0, earn your way up)\n"
        "Score each pick 1.0-5.0. **Start at 1.0** and ADD points only for concrete evidence:\n"
        "  +0.5 — You found a specific ATS record that supports this side (e.g. '7-2 ATS last 9')\n"
        "  +0.5 — Clear injury/absence creates a real edge (star player out, not just 'questionable')\n"
        "  +0.5 — PPG/pace data clearly supports direction (combined projection 6+ pts away from line for O/U)\n"
        "  +0.5 — Strong situational factor (rest advantage, road/home splits, revenge game with data)\n"
        "  +0.5 — Multiple independent factors align (not just restating the same edge twice)\n"
        "  +0.5 — Historical H2H or specific matchup data backs it up\n"
        "A typical pick with one decent reason = 1.5-2.0. A pick with 2-3 solid, cited reasons = 2.5-3.5. "
        "Only a pick where everything lines up (4+ factors with real data) should reach 4.0+. "
        "Most games on most nights should land 1.5-3.5. A 4.0+ means you'd bet your own money.\n\n"
...
async def analyze_game(req: AnalyzeRequest):
    ...
    if is_live:
        prompt = (
            ...
            "DUBL_SCORE_BET: [float 1.0-5.0. Start at 1.0. Add +0.5 for clear score margin, "
            "+0.5 for momentum/run, +0.5 for key player performance edge, +0.5 for situational factor (foul trouble, fatigue), "
            "+0.5 if multiple factors align. Tight game with no clear edge = 1.5-2.0.]\n"
            "DUBL_REASONING_BET: [1 sentence about the EXACT bet — reference the live score situation, momentum, or specific matchup factor.]\n"
            "DUBL_SCORE_OU: [float 1.0-5.0. Start at 1.0. Add +0.5 if current pace projects 8+ pts from line, "
            "+0.5 for quarter-by-quarter trend supporting direction, +0.5 for key player scoring pace, "
            "+0.5 if multiple factors align. Pace projection close to line = 1.5-2.0.]\n"
        ...
        prompt = (
            ...
            "DUBL_SCORE_BET: [float 1.0-5.0. Start at 1.0 and add +0.5 per concrete factor: "
            "ATS record, injury edge, situational factor, matchup data, multiple independent factors aligning. "
            "A pick with one decent reason = ~2.0. Two solid reasons = ~2.5-3.0. Only 4+ factors = 4.0+.]\n"
            ...
            "DUBL_SCORE_OU: [float 1.0-5.0. Start at 1.0. Add +0.5 if combined PPG projection is 6+ away from line, "
            "+0.5 for pace/defensive rating support, +0.5 for injury/rest factor, +0.5 for H2H totals trend, "
            "+0.5 if multiple independent factors align. Projection close to line = stay near 1.5-2.0.]\n"
