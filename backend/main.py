...
def build_system_prompt(
    ...
    return (
        ...
        "### DUBL SCORES — MUST VARY ACROSS GAMES\n"
        "You will score each pick 1.0-5.0. The ENTIRE POINT of these scores is to help users prioritize "
        "which 1-2 games to bet on tonight. If every game gets the same score, the scores are worthless. "
        "Use the full range with 0.1 precision (e.g. 2.3, 3.7, 4.1 — NOT just round numbers). "
        "On a typical night: 1-2 games should be your HIGH conviction (4.0+), 2-3 games MEDIUM (3.0-3.8), "
        "and 1-2 games LOW conviction (1.5-2.8). If you lack strong evidence for a pick, score it LOW — "
        "that honesty is more valuable than inflating everything to 4.0.\n\n"
...
async def analyze_game(req: AnalyzeRequest):
    ...
    if is_live:
        prompt = (
            ...
            "DUBL_SCORE_BET: [float 1.0-5.0 in 0.1 increments (e.g. 2.3, 3.7, 4.1). "
            "Differentiate across live games — which has the clearest edge RIGHT NOW? "
            "Score based on live situation strength: blowout with clear side = high, tight game with uncertain momentum = low.]\n"
            "DUBL_REASONING_BET: [1 sentence about the EXACT bet — reference the live score situation, momentum, or specific matchup factor.]\n"
            "DUBL_SCORE_OU: [float 1.0-5.0 in 0.1 increments. Score higher when current pace clearly projects over/under the line, "
            "lower when the projection is close to the line.]\n"
        ...
        prompt = (
            ...
            "DUBL_SCORE_BET: [float 1.0-5.0 in 0.1 increments (e.g. 2.3, 3.7, 4.1). "
            "This score ranks how confident you are in THIS pick relative to all other games tonight. "
            "CRITICAL: You must differentiate! On a typical 6-game night, a realistic distribution is: "
            "one or two games at 1.5-2.5 (weak/coin flip), two or three at 3.0-3.8 (decent lean), "
            "one or two at 4.0-4.8 (strong conviction). 5.0 is extremely rare (1-2 per month). "
            "If you give every game the same score, the scores are useless. Ask yourself: "
            "which of tonight's games do I have the MOST evidence for and which am I least sure about? "
            "Score those differently. Use the full range — a 2.1 is fine if you're genuinely unsure.]\n"
            ...
            "DUBL_SCORE_OU: [float 1.0-5.0 in 0.1 increments. Same rules: differentiate across games. "
            "O/U picks backed by clear PPG vs line mismatch (e.g. combined PPG is 8+ above/below the line) score higher. "
            "O/U picks where the PPG projection is close to the line score lower (2.0-3.0). Be honest about uncertainty.]\n"
